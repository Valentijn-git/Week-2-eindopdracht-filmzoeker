...
//document.getElementById("avengersBtn").click()



//const linkPosters = movies.map((movies) => {
//    array.forEach(Poster => {
//        posterUl.appendChild(newLi.appendChild(newImg.setAttribute("src", posterURL)))
//    });
//});

//const posterURL = movies.map((movies) => {
//    movies.getElementsByTagName("Poster")
//});

const posterUl = movies.map((movies) => {
    return movies.Poster
});

console.log(posterUl);
